...
from ...formulations.bilevel_test_helper import create_sobieski_bilevel_bcd_scenario
from ...formulations.bilevel_test_helper import create_sobieski_bilevel_scenario
...
def deterministic_slsqp(monkeypatch):
    ...
    monkeypatch.setattr(
        "gemseo.algos.opt.scipy_local.scipy_local.ScipyOpt._run",
        mock_run,
    )
...
def test_executable_discipline(dm_settings):
    ...
    root_path = dm_settings.execution_root_path
    file_path = Path(__file__).parent.parent.parent / "disciplines" / "wrappers"
