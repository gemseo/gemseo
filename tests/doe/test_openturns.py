...
from gemseo.doe.openturns.settings.base_ot_stratified_doe import (
...
if TYPE_CHECKING:
    from gemseo.doe.openturns._algorithm.base_ot_stratified_doe import (
        BaseOTStratifiedDOE,
    )
...
def test_ot_stratified_doe_n_samples(cls, n_samples):
    ...
    assert cls().generate_samples(
        3, BaseOTStratifiedDOESettings(n_samples=n_samples)
    ).shape == (n_samples, 3)
...
def test_ot_stratified_doe_n_samples_warning(cls, n_samples, caplog, error_message):
    ...
    # We use d=3 and l=4.
    algo = cls()
    samples = algo.generate_samples(3, BaseOTStratifiedDOESettings(n_samples=n_samples))
    ...
    assert_equal(
        samples,
        cls().generate_samples(3, BaseOTStratifiedDOESettings(n_samples=n_samples - 1)),
    )
...
def test_ot_stratified_results(cls, centers, levels, expected_samples):
    ...
    samples = cls().generate_samples(
        2, BaseOTStratifiedDOESettings(centers=centers, levels=levels)
    )
