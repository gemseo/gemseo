...
def test_wing_weight_space() -> None:
    ...
    for distribution, loc, scale in zip(
        space.variables.values(), loc_values, scale_values, strict=False
    ):
        assert len(distribution.distribution.marginals) == 1
        distribution = distribution.distribution.marginals[0].distribution
