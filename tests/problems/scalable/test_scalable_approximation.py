...
from gemseo.problems.scalable.diagonal import ScalableDiagonalApproximation
...
def test_build_model(sobieski_aerodynamics):
    ...
    scd = ScalableDiscipline("ScalableDiagonalModel", hdf_cache, sizes, fill_factor=0.7)
    comp_dep, in_dep = scd.scalable_model.generate_random_dependency()

    all_data = hdf_cache.get_all_data(True)
    data = next(all_data)
    input_data = data[hdf_cache.INPUTS_GROUP]

    var_lb = {varname: value for varname, value in input_data.items()}
    var_ub = {varname: value for varname, value in input_data.items()}
    for data in all_data:
        input_data = data[hdf_cache.INPUTS_GROUP]
        for varname, value in input_data.items():
            var_lb[varname] = np.minimum(var_lb[varname], value)
            var_ub[varname] = np.maximum(var_ub[varname], value)

    scale_pb = ScalableDiagonalApproximation(sizes, var_lb, var_ub, comp_dep, in_dep)
    scale_pb.build_scalable_function("y_23", hdf_cache, ["x_shared"])
    ...
    scd = ScalableDiscipline("ScalableDiagonalModel", hdf_cache, sizes, fill_factor=0.0)
    comp_dep, in_dep = scd.scalable_model.generate_random_dependency()
    scale_pb = ScalableDiagonalApproximation(sizes, var_lb, var_ub, comp_dep, in_dep)
    scale_pb.build_scalable_function("y_23", hdf_cache, ["x_shared"])
    ...
    scd = ScalableDiscipline("ScalableDiagonalModel", hdf_cache, sizes, fill_factor=0.7)
    comp_dep, in_dep = scd.scalable_model.generate_random_dependency()
    ScalableDiagonalApproximation(sizes, var_lb, var_ub, comp_dep, in_dep)


def test_serialize(tmp_path, sobieski_aerodynamics):
    """Test the serialization of a SobieskiAerodynamics instance."""
    s_file = str(tmp_path / "aero.o")
