...
def test_lhs_seed():
    lhs = PyDOELibrary("PYDOE_LHS")
    reference = lhs.sample_unit_hypercube(2, PYDOE_LHS_Settings(n_samples=3))
    # Same seed, same samples.
    assert_equal(
        lhs.sample_unit_hypercube(2, PYDOE_LHS_Settings(n_samples=3, seed=lhs.seed)),
        reference,
    )
    assert_equal(
        lhs.sample_unit_hypercube(2, PYDOE_LHS_Settings(n_samples=3, seed=2)),
        lhs.sample_unit_hypercube(2, PYDOE_LHS_Settings(n_samples=3, seed=2)),
    )
    # Different seeds, different samples.
    assert not array_equal(
        lhs.sample_unit_hypercube(2, PYDOE_LHS_Settings(n_samples=3, seed=2)), reference
    )
    assert not array_equal(
        lhs.sample_unit_hypercube(
            2, PYDOE_LHS_Settings(n_samples=3, seed=default_rng(seed=123))
        ),
        reference,
    )
