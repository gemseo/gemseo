...
if TYPE_CHECKING:
    from gemseo.algos.doe.openturns._algos.base_ot_stratified_doe import (
        BaseOTStratifiedDOE,
    )
...
def test_ot_stratified_doe_n_samples_error(cls, error_message):
    ...
    with pytest.raises(ValueError, match=re.escape(error_message)):
        cls().generate_samples(1, 2, settings=BaseOTStratifiedDOESettings(n_samples=4))
...
def test_ot_stratified_doe_n_samples(cls, n_samples):
    ...
    # We use d=3 and l=4.
    assert cls().generate_samples(
        1, 3, settings=BaseOTStratifiedDOESettings(n_samples=n_samples)
    ).shape == (n_samples, 3)
...
def test_ot_stratified_doe_n_samples_warning(cls, n_samples, caplog, error_message):
    ...
    assert_equal(
        samples,
        cls().generate_samples(
            1, 3, settings=BaseOTStratifiedDOESettings(n_samples=n_samples - 1)
        ),
    )
...
def test_ot_stratified_centers_dimension_error(cls: BaseOTStratifiedDOE):
    ...
    with pytest.raises(
        ...
        cls().generate_samples(
            1, 3, settings=BaseOTStratifiedDOESettings(centers=[0.5, 0.6])
        )
...
def test_ot_stratified_centers_value_error(cls, centers):
    ...
    with pytest.raises(
        ...
        cls().generate_samples(
            1, 2, settings=BaseOTStratifiedDOESettings(centers=centers)
        )
...
def test_ot_stratified_levels_value_error(cls):
    ...
    with pytest.raises(
        ...
        cls().generate_samples(
            1, 2, settings=BaseOTStratifiedDOESettings(levels=[0, 0.5])
        )
...
def test_ot_stratified_results(cls, centers, levels, expected_samples):
    ...
    samples = cls().generate_samples(
        1, 2, settings=BaseOTStratifiedDOESettings(centers=centers, levels=levels)
    )
