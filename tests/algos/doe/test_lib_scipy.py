...
def test_remove_recent_scipy_options(library):
    ...
    _version = version.parse(scipy.__version__)
...
def check_option_filtering(option_name, target_version, current_version, caplog):
    ...
    is_old_version = version.parse(current_version) < version.parse(target_version)
    assert (text in caplog.text) is is_old_version


@pytest.mark.parametrize("algo_name", ["Sobol", "Halton", "MC", "LHS", "PoissonDisk"])
@pytest.mark.parametrize("__version__", ["1.7", "1.8", "1.9", "1.10"])
@pytest.mark.parametrize("seed", [None, 3])
def test_generate_samples(library, algo_name, __version__, seed, caplog):
    ...
    library._SciPyDOE__SCIPY_VERSION = version.parse(__version__)
    samples = library._generate_samples(dimension=dimension, **options)
    library._SciPyDOE__SCIPY_VERSION = scipy_version

    if algo_name == "Sobol":
        check_option_filtering("bits", "1.9", __version__, caplog)
        check_option_filtering("optimization", "1.10", __version__, caplog)
    elif algo_name == "Halton":
        check_option_filtering("optimization", "1.10", __version__, caplog)
    elif algo_name == "LHS":
        check_option_filtering("scramble", "1.10", __version__, caplog)
        check_option_filtering("optimization", "1.8", __version__, caplog)
        check_option_filtering("strength", "1.8", __version__, caplog)
