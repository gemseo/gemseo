...
from gemseo.uncertainty.distribution.scipy import distribution_fitter
...
def test_plot(statistics, snapshot_matplotlib) -> None:
...
def test_plot_html(statistics, snapshot_allclose):
    """Check the visualizations generated by the method plot() for html figures."""
    pytest.importorskip("plotly")
    graphs = statistics.plot(show=False, file_format="html")
    for graph in graphs.values():
        assert json.loads(graph.to_json()) == snapshot_allclose(rtol=1e-3)
...
def test_plot_criteria_images(
    dataset, fitting_criterion, title, snapshot_matplotlib
) -> None:
    ...
    statistics.plot_criteria("x_1", show=False, save=False, title=title)
