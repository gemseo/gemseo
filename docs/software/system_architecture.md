...
- [MDOScenario][gemseo.scenario.mdo.MDOScenario] builds the process from a set of inputs, several disciplines and a formulation. It is one of the main interface class for the MDO user. The [MDOScenario][gemseo.scenario.mdo.MDOScenario] triggers the overall optimization process when its [execute()][gemseo.scenario.mdo.MDOScenario.execute] method is called. Through this class, the user provides an initial solution, user constraints, and bounds on the design variables. The user may also generate visualization of the scenario execution, such as convergence plots of the algorithm.
- [Discipline][gemseo.core.discipline.discipline.Discipline] represents a wrapped simulation software program or a chain of wrapped software. It can either be a link to a discipline integrated within a workflow engine, or can be inherited to integrate a simulation software directly. Its inputs and outputs are represented in a **Grammar** (see [SimpleGrammar][gemseo.core.grammar.simple.SimpleGrammar] or [JSONGrammar][gemseo.core.grammar.json.JSONGrammar]).
- [BaseMDOFormulation][gemseo.formulation.core.base_mdo.BaseMDOFormulation] describes the MDO formulation (*e.g.* MDF and IDF) used by the [MDOScenario][gemseo.scenario.mdo.MDOScenario] to generate the [OptimizationProblem][gemseo.optimization.problem.OptimizationProblem]. The MDO user or the MDO integrator may either provide the name of the formulation, or a class, or an instance, to the [MDOScenario][gemseo.scenario.mdo.MDOScenario]. The MDO formulations designer may create, implement, test or maintain MDO formulations with this class.
- [OptimizationProblem][gemseo.optimization.problem.OptimizationProblem] describes the mathematical functions of the optimization problem (objective function and constraints, along with the design variables. It is generated by the [BaseMDOFormulation][gemseo.formulation.core.base_mdo.BaseMDOFormulation], and solved by the optimization algorithm. It has an internal database that stores the calls to its functions by the optimization algorithm to avoid duplicate computations. It can be stored on disk and analyzed *a posteriori* by post-processing available in GEMSEO.
- `design_space` is a [DesignSpace][gemseo.space.design.DesignSpace] attribute of the [OptimizationProblem][gemseo.optimization.problem.OptimizationProblem] that describes the design variables, their bounds, their type (real or integer), and current value. This object can be read from a file.

Two low-level classes at the core of GEMSEO are crucial for the understanding of its basic principles:

- [ArrayFunction][gemseo.core.function.array_function.ArrayFunction] instances (for the objective function and the possible constraints) are generated by the [BaseMDOFormulation][gemseo.formulation.core.base_mdo.BaseMDOFormulation]. Depending on the formulation, constraints may be generated as well (*e.g.* consistency constraints in IDF).
- [DisciplineAdapterGenerator][gemseo.core.function.discipline_adapter_generator.DisciplineAdapterGenerator] is a utility class that handles the [ArrayFunction][gemseo.core.function.array_function.ArrayFunction] generation for a given [Discipline][gemseo.core.discipline.discipline.Discipline]. It is a key class for the MDO formulations designer.
