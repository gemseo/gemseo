---
description: Use MATLAB for disciplines.
status: deployed
comment: TODO - this page should appear in the MATLAB plugin
---
...
As explained in [Interfacing simulation software](../interface/software_connection.md), GEMSEO can interface any simulation software through the [Discipline][gemseo.core.discipline.discipline.Discipline] class. When dealing with a MATLAB program given as a function, a generic interface is created in order to facilitate this task. It is described here.
...
In order to use the [MatlabDiscipline][gemseo_matlab.matlab_discipline.MatlabDiscipline], you shall install the MATLAB plugin for GEMSEO. Please refer to the plugin installation instructions.
...
!!! note
    If the discipline is executed with inputs that have the wrong size,
    an error is raised.
...
More generally, if the basis function takes an input vector $\bf{x}$ and returns an output vector $\bf{y}$, the total derivatives denoted $\frac{d\bf{f}}{d\bf{x}}$ is called the Jacobian matrix as explained in [Coupled derivatives and gradients computation](jacobian_assembly.md).
...
One can see that the Jacobian outputs are not included in the returned values. Since the argument `is_jac_returned_by_func` has been activated, the Jacobian matrices values are stored in the [Discipline.jac][gemseo.core.discipline.discipline.Discipline.jac] attribute. Thus printing [Discipline.jac][gemseo.core.discipline.discipline.Discipline.jac] in a pretty way gives:
...
As explained in the section [Overloading the `Discipline._compute_jacobian()` method](discipline_from_scratch.md#overloading-the-discipline_compute_jacobian-method), GEMSEO always manipulates the Jacobian terms inside 2D arrays even if the Jacobian is reduced to a scalar value, row-vector or column-vector values.
...
Many others optional parameters can be added when building a MATLAB discipline. They are all listed in the description of [MatlabDiscipline][gemseo_matlab.matlab_discipline.MatlabDiscipline] but we give some information here about the most important ones.
...
The arguments `input_names` and `output_names` enable to change the name of the input and/or output variables when using the discipline. As an example, in the previous simple scalar case, the inputs and outputs are respectively denoted `x`, `y`, `z1` and `z2` in the MATLAB function:
...
When building a MATLAB discipline, the MATLAB Python API launches a MATLAB workspace that will be used in order to execute the MATLAB function that is wrapped. MATLAB workspace handling is done through the [MatlabEngine][gemseo_matlab.engine.MatlabEngine] class. Since this class is private, it cannot be imported directly form the module. An instance of this class is rather obtained through the function [get_matlab_engine()][gemseo_matlab.engine.get_matlab_engine] which acts like a singleton. This means that calling `.get_matlab_engine`{.interpreted-text role="func"} with the same input argument (the workspace name), returns exactly the same instance. Therefore, if one builds two disciplines, they will be executed in a unique MATLAB workspace. This is indeed what a MATLAB user do when working with MATLAB: run MATLAB once and execute any function inside the same environment.

The uniqueness of the [MatlabEngine][gemseo_matlab.engine.MatlabEngine] instance depends more precisely on the workspace name that is passed to the function [get_matlab_engine()][gemseo_matlab.engine.get_matlab_engine]: when getting two engines, if the names are the same then the instance is unique, otherwise they are not. Let's see the following simple example with three engines, two based on the same name and the third based on a different one:
...
This `workspace_name` string that is passed to the [get_matlab_engine()][gemseo_matlab.engine.get_matlab_engine] can be controlled with the argument `matlab_engine_name` when building the MATLAB discipline from GEMSEO API. By default, this argument is set to `"matlab"` and should not be changed except for very specific use.
