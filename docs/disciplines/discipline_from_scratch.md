...
Creating a discipline from scratch implies to implement
a new class inheriting from [Discipline][gemseo.core.discipline.discipline.Discipline].

For example,
let's consider a discipline called `NewDiscipline`,
with two outputs, `f` and `g`,
and two inputs, `x` and `z`,
where `f=x*z` and `f=x*(z+1)^2`.

## Overloading the [Discipline][gemseo.core.discipline.discipline.Discipline]'s constructor

First of all, we overload the [Discipline][gemseo.core.discipline.discipline.Discipline] constructor.
For that, we call the [Discipline][gemseo.core.discipline.discipline.Discipline] superconstructor:
...
Then, we define the [input_grammar][gemseo.core.discipline.discipline.Discipline.input_grammar] and [output_grammar][gemseo.core.discipline.discipline.Discipline.output_grammar] created by the base class constructor. We have different ways to do that.

#### Setting the grammars from data names

When the variables are float arrays without any particular constraint, the simplest approach is to apply the [BaseGrammar.update_from_names][gemseo.core.grammars.base_grammar.BaseGrammar.update_from_names] method to a list of variable names:
...
An intermediate approach is to apply the [BaseGrammar.update_from_data][gemseo.core.grammars.base_grammar.BaseGrammar.update_from_data] method with a `dict` data example:
...
Discipline inputs and outputs shall be [numpy](http://www.numpy.org/) arrays of real numbers or integers.
...
We also define the default inputs by means of the [Discipline.default_input_data][gemseo.core.discipline.discipline.Discipline.default_input_data] attribute:
...
Once the input and output have been declared in the constructor of the discipline, the abstract `Discipline._run()` method of [Discipline][gemseo.core.discipline.discipline.Discipline] shall be implemented by the discipline to define how outputs are computed from inputs.

!!! info "See Also"

    The method is protected (starts with "_") because it shall not be called from outside the discipline. External calls that trigger the discipline execution use the [execute()][gemseo.core.discipline.discipline.Discipline.execute] public method from the base class, which provides additional services before and after calling `Discipline._run()`. These services, such as data checks by the grammars, are provided by GEMSEO and the integrator of the discipline does not need to implement them.
...
The [Discipline][gemseo.core.discipline.discipline.Discipline] may also provide the derivatives of their outputs with respect to their inputs, i.e. their Jacobians. This is useful for gradient-based optimization or [Multi Disciplinary Analyses](../mdo/mda.md) based on the Newton method. For a vector of inputs $x$ and a vector of outputs $y$, the Jacobian of the discipline is $\frac{\partial y}{\partial x}$.

The discipline shall provide a method to compute the Jacobian for a given set of inputs. This is made by overloading the abstract `Discipline._compute_jacobian()` method of [Discipline][gemseo.core.discipline.discipline.Discipline]. The discipline may have multiple inputs and multiple outputs. To store the multiple Jacobian matrices associated to all the inputs and outputs, GEMSEO uses a dictionary of dictionaries structure. This data structure is sparse and makes easy the access and the iteration over the elements of the Jacobian.
