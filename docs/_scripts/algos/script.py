...
def get_options_schemas(
    ...
    for _opt_name, opt_schema in list(all_options.items()):
        ...
        if obj_type in opt_schema:
            ...
            var_types = [
                sub_opt_schema[obj_type].replace("\n", " ")
...
class AlgoOptionsDoc:
    ...
    def __init__(
        ...
        template: The name of the template file located in the same directory
            as the current file.
            If None, `AlgoOptionsDoc.TEMPLATE` will be used.
        ...
        if hasattr(self.factory, "class_names"):
            class_names = []
            for class_name in algo_factory.class_names:
                cls = algo_factory.get_class(class_name)
                if class_name.startswith("_") or any(
                    module_name.startswith("_")
                    for module_name in cls.__module__.split(".")
                ):
                    continue
                class_names.append(class_name)
            self.algos_names = class_names
        else:
    ...
    def to_rst(
        ...
        Args:
            ...
            output_file_name: The name of the rST output file.
                If None, the name will be "{algo_type}_algos.md".
        """
        if template_file_name is None:
            template_file_name = self.template

        if output_file_name is None:
            output_file_name = f"{self.algo_type}_algos.md"
        ...
        with (root / output_file_name).open("w", encoding="utf-8") as f:
            f.write(doc)
