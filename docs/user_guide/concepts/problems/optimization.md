...
extends an [EvaluationProblem][gemseo.core.problem.evaluation.EvaluationProblem]
by introducing an *objective* function to minimize or maximize
and optional *constraints* that solutions must satisfy.
It operates on a [DesignSpace][gemseo.space.design.DesignSpace]
...
The objective is an [ArrayFunction][gemseo.core.function.array_function.ArrayFunction]
...
are [ArrayFunction][gemseo.core.function.array_function.ArrayFunction] instances
added via [add_constraint()][gemseo.optimization.problem.OptimizationProblem.add_constraint].
They are internally rewritten in the general form mentioned above.
Feasibility is checked against configurable
[ConstraintTolerances][gemseo.optimization.constraint_tolerances.ConstraintTolerances]
accessible through the `tolerances` attribute.

An optimization algorithm is typically used to solve an [OptimizationProblem][gemseo.optimization.problem.OptimizationProblem],
...
Once solved,
the `solution` attribute holds an [OptimizationResult][gemseo.optimization.result.OptimizationResult]
...
with its [Database][gemseo.core.problem.database.Database] can be exported to an HDF file
using [to_hdf()][gemseo.optimization.problem.OptimizationProblem.to_hdf],
and restored using [from_hdf()][gemseo.optimization.problem.OptimizationProblem.from_hdf].

[OptimizationProblem][gemseo.optimization.problem.OptimizationProblem] does with [functions][concept-functions]
what [MDOScenario][gemseo.scenario.mdo.MDOScenario] does with [disciplines][concept-discipline].
