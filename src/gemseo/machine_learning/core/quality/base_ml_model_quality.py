...
"""Measuring the quality of a machine learning model."""
...
class BaseMLModelQuality(metaclass=ABCGoogleDocstringInheritanceMeta):
    """The base class to assess the quality of a machine learning model.

    This measure can be minimized
    (e.g.
    [MSEMeasure][gemseo.machine_learning.regression.quality.mse_measure.MSEMeasure])
    or maximized
    (e.g. [R2Measure][gemseo.machine_learning.regression.quality.r2_measure.R2Measure]).
    ...
    The machine learning model is usually trained. If not but required by the
    evaluation technique, the quality measure will train it.

    Lastly, the transformers of the model fitted from the training dataset can be
    used as is by the resampling methods or re-fitted for each model trained on a
    subset of the training dataset.
    """

    model: BaseMLModel
    """The machine learning model whose quality we want to measure."""
    ...
    def __init__(
        ...
        Args:
            ...
            fit_transformers: Whether to re-fit the transformers
                ...
                If `False`,
                use the transformers of the model fitted
    ...
    def compute_leave_one_out_measure(
        ...
        Args:
            ...
            store_resampling_result: Whether to store
                the $n$ machine learning models and associated predictions
        ...
        return self.compute_cross_validation_measure(
            samples=samples,
            n_folds=len(self.model.learning_set),
    ...
    def _pre_process(
        ...
        if not self.model.is_trained:
            self.model.learn(samples)

        if not samples:
            samples = self.model.learning_samples_indices
