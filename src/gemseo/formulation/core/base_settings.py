...
More precisely,
for each function,
an [ArrayFunction][gemseo.core.function.array_function.ArrayFunction]
is built from the `disciplines`,
which depend on input variables $x_1,\ldots,x_d,x_{d+1}$,
and over an input space
spanned by the input variables $x_1,\ldots,x_d$
and depending on both the MDO formulation and the `design_space`.
Then,
the methods
[ArrayFunction.evaluate()][gemseo.core.function.array_function.ArrayFunction.evaluate]
and
[ArrayFunction.jac()][gemseo.core.function.array_function.ArrayFunction.jac]
...
with respect to the inputs $x_1,\ldots,x_d$
