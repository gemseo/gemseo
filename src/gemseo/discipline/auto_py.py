...
class AutoPyDiscipline(Discipline):
    ...
    - must have a default value per argument
      if the [AutoPyDiscipline][gemseo.discipline.auto_py.AutoPyDiscipline]
      is used by an `MDA`
      (deriving from [BaseMDA][gemseo.mda.core.base.BaseMDA]),
      as in the case of [MDF][gemseo.formulation.mdf.MDF]
      and [BiLevel][gemseo.formulation.bilevel.BiLevel] formulations,
