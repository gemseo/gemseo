...
class SensitivityAnalysis(object):
    ...
    def plot_bar(
        ...
        **options  # type:int
        ...
        plot.title = title
        plot.execute(save, show, file_path, file_format, **options)
        return plot
    ...
    def _save_show_plot(
        ...
        Args:
            ...
            file_format: A file format, e.g. 'png', 'pdf', 'svg', ...
                ...
                If None, 'pdf' format is considered.
        """
        default_file_name = findall("[A-Z][^A-Z]*", self.__class__.__name__)
        default_file_name = "_".join([elem.lower() for elem in default_file_name])
        file_path = make_fpath(default_file_name, file_path, file_format)
        if save:
