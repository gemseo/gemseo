...
class OptimizationLibrary(DriverLib):
    """Optimization library base class
    See DriverLib
    ...
    def is_algo_requires_positive_cstr(self, algo_name):
        """Returns True if the algorithm requires positive constraints
        False otherwise
    ...
    def _check_constraints_handling(self, algo_name, problem):
        """Check if problem and algorithm are consistent for
    ...
    def get_right_sign_constraints(self):
        """Transforms the problem constraints into their opposite sign
        counterpart if the algorithm requires positive constraints
    ...
    def _pre_run(self, problem, algo_name, **options):
        ...
        elif self.OPTIONS_MAP[self.MAX_ITER] in options:
            max_iter = options[self.OPTIONS_MAP[self.MAX_ITER]]
        else:
            raise ValueError("Could not determine maximum " + "number of iterations.")
        problem.log_me(self.MAX_DS_SIZE_PRINT)
        self.init_progress_bar(max_iter, "Optimization")
        problem.add_callback(self.iterate_progress_bar)
    ...
    def is_algorithm_suited(algo_dict, problem):
        ...
        lin_alg = "pb_type" in algo_dict and algo_dict["pb_type"] == problem.LINEAR_PB
