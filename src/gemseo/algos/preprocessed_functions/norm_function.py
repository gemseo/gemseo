...
class NormFunction(NormFunctionMixin, BasePreprocessedFunction):
    """A function with normalized inputs."""
    ...
    def _compute_output(self, x_vect: ndarray) -> ndarray:
        return self._original_func(self.__unnormalize_vect(x_vect))

    def _compute_jacobian(self, x_vect: ndarray) -> ndarray:
        return self.__normalize_grad(
            self._original_jac(self.__unnormalize_vect(x_vect))
        )
