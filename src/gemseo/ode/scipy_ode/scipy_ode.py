...
from gemseo.ode.scipy_ode.settings.base_scipy_ode_jac_settings import (
    BaseScipyODESolverJacSettings,
)
from gemseo.ode.scipy_ode.settings.bdf import BDF_Settings
from gemseo.ode.scipy_ode.settings.dop853 import DOP853_Settings
from gemseo.ode.scipy_ode.settings.lsoda import LSODA_Settings
from gemseo.ode.scipy_ode.settings.radau import Radau_Settings
from gemseo.ode.scipy_ode.settings.rk23 import RK23_Settings
...
class ScipyODEAlgos(BaseODESolverLibrary):
    ...
    def _run(self, problem: ODEProblem) -> ODEResult:
        ...
        if problem.event_functions and index is not None:
            problem.result.termination_time = float(solution.t_events[index][0])
