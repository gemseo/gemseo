...
def image_comparison(
    *args: Any, **kwargs: Any
    ...
    This overloads `matplotlib.testing.decorators.image_comparison` by using
    `"default"` as `style` if missing. Use `["png"]` as `extensions` if missing.

    Also add the mark `post` to the corresponding tests.
    """
    if "style" not in kwargs:  # pragma: no cover
        kwargs["style"] = "default"
    if "extensions" not in kwargs:
        kwargs["extensions"] = ["png"]
    wrapped = mpl_image_comparison(*args, **kwargs)

    def __add_post_mark(func):
        return pytest.mark.post(wrapped(func))

    return __add_post_mark
