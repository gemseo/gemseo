...
"""The type of a [Database][gemseo.core.problem.database.Database] key."""

FunctionOutputValueType = float | ndarray | list[int]
"""The type of a function output value stored in a [Database][gemseo.core.problem.database.Database]."""  # noqa: E501

DatabaseValueType = Mapping[str, FunctionOutputValueType]
"""The type of a [Database][gemseo.core.problem.database.Database] value."""

ListenerType = Callable[[DatabaseKeyType], None]
"""The type of a listener attached to an
...
class Database(Mapping):
    ...
    A [Database][gemseo.core.problem.database.Database] is typically attached
    o an [OptimizationProblem][gemseo.optimization.problem.OptimizationProblem]
    to store the evaluations of its objective, constraints and observables.

    Then,
    a [Database][gemseo.core.problem.database.Database] can be an optimization history
    ...
    It can also be post-processed by a [BasePost][gemseo.post.core.base_post.BasePost]
    ...
    A [Database][gemseo.core.problem.database.Database] can be saved to an HDF file
    for portability and cold post-processing
    with its method [to_hdf()][gemseo.core.problem.database.Database.to_hdf].
    ...
    Note:
        ...
        to an HDF file
        using its method
        [to_hdf][gemseo.optimization.problem.OptimizationProblem.to_hdf]
        also saves its [Database][gemseo.core.problem.database.Database].

    The database is based on a two-level dictionary-like mapping such as
    `{x: {output_name: output_value, ...}, ...}` with:

        * `x`: the input value
          as an [HashableNdarray][gemseo.util.hashable_ndarray.HashableNdarray]
    ...
    def get_hashable_ndarray(
        ...
        Raises:
            KeyError: If the original array is
                neither an array
                nor a [HashableNdarray][gemseo.util.hashable_ndarray.HashableNdarray].
        """
