...
class Value(RegistryDerivedData):
    ...
    def initialize_missing(self) -> None:
        ...
        for name, variable in self._variables.items():
            ...
            self.set_variable(name, variable.get_default_value())
    ...
    def __format_values(
        ...
        """Cast variable values to real numbers if requested, as new arrays.

        The values are copied,
        as is the dictionary holding them,
        so that a caller mutating what this method returns
        cannot make the space inconsistent.
        ...
        """
        if complex_to_real:
            # The real part of an array is a view of it, so it is copied too.
            return {name: value.real.copy() for name, value in name_to_value.items()}

        return {name: value.copy() for name, value in name_to_value.items()}

    @staticmethod
    def __format_full_value(value: NumberArray, complex_to_real: bool) -> NumberArray:
        """Cast a full value to real if requested, as a new array.

        The value is copied,
        so that a caller mutating it cannot make the space inconsistent.
