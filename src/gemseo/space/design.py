...
A [DesignSpace][gemseo.space.design.DesignSpace] describes this design space
at a given state,
in terms of names, sizes, types, bounds and current values of the design variables.

Variables can easily be added
to the [DesignSpace][gemseo.space.design.DesignSpace]
using the
...
Lastly,
an instance of [DesignSpace][gemseo.space.design.DesignSpace] can be stored
...
class DesignSpace:
    ...
    def to_file(
        ...
        if file_path.suffix.startswith((".hdf", ".h5")):
            self.to_hdf(file_path, append=append)
        else:
            self.to_csv(file_path, delimiter=delimiter, fields=fields)
    ...
    def to_csv(
        ...
        dataframe.to_csv(
            ...
            sep=delimiter or " ",
            index=False,
            columns=fields or self.TABLE_NAMES,
            na_rep="None",
        )
