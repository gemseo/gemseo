...
The [ParameterSpace][gemseo.space.parameter.ParameterSpace] class describes
a set of parameters of interest
which can be either deterministic or uncertain.
This class inherits from [DesignSpace][gemseo.space.design.DesignSpace].
...
The [ParameterSpace][gemseo.space.parameter.ParameterSpace] also provides
...
- [get_support()][gemseo.space.parameter.ParameterSpace.get_support]:
...
- [is_deterministic()][gemseo.space.parameter.ParameterSpace.is_uncertain]:
...
class ParameterSpace(DesignSpace):
    ...
    def normalize_vect(
        ...
        Otherwise,
        use the approach defined in
        [DesignSpace.normalize_vect()][gemseo.space.design.DesignSpace.normalize_vect]
        with `minus_lb`.
    ...
    def extract_uncertain_space(
        ...
        Args:
            as_design_space: If `False`,
                return a [ParameterSpace][gemseo.space.parameter.ParameterSpace]
                containing the original uncertain variables as is;
                otherwise,
                return a [DesignSpace][gemseo.space.design.DesignSpace]
