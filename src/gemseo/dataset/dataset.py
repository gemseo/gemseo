...
A [Dataset][gemseo.dataset.dataset.Dataset] is a pandas
...
A [Dataset][gemseo.dataset.dataset.Dataset] can be set
either from a file (
[from_csv()][gemseo.dataset.dataset.Dataset.from_csv]
and [from_txt()][gemseo.dataset.dataset.Dataset.from_txt]
...
A [BaseFullCache][gemseo.core.cache.base_full.BaseFullCache]
or an [OptimizationProblem][gemseo.optimization.problem.OptimizationProblem]
can also be exported to a [Dataset][gemseo.dataset.dataset.Dataset]
using the methods
[BaseFullCache.to_dataset()][gemseo.core.cache.base_full.BaseFullCache.to_dataset]
and
...
class Dataset(DataFrame, metaclass=GoogleDocstringInheritanceMeta):
    ...
    [Dataset][gemseo.dataset.dataset.Dataset] is a collection of entries
    corresponding to a set of variable identifiers.
    An entry corresponds to an index of the [Dataset][gemseo.dataset.dataset.Dataset].

    A [Dataset][gemseo.dataset.dataset.Dataset] is a special pandas DataFrame
    ...
    [from_csv()][gemseo.dataset.dataset.Dataset.from_csv],
    and [from_dataframe()][gemseo.dataset.dataset.Dataset.from_dataframe].

    Miscellaneous information that is not specific to an entry of the dataset
    can be stored in the dictionary [misc][gemseo.dataset.dataset.Dataset.misc],
    as `dataset.misc["year"] = 2023`.

    Notes:
        The columns of a data structure
        (NumPy array, `DataFrame`, [Dataset][gemseo.dataset.dataset.Dataset], ...)
        are called *features*.
        The features of a [Dataset][gemseo.dataset.dataset.Dataset] include
        all the components of all the variables of all the groups.

    Warning:
        A [Dataset][gemseo.dataset.dataset.Dataset] behaves
    ...
    def from_csv(
        ...
        See Also:
            ...
            the method [from_txt()][gemseo.dataset.dataset.Dataset.from_txt]
            is better suited.
    ...
    def summary(self) -> str:
        ...
        string.indent()
        for group_name, group_size in self.group_name_to_n_components.items():
