...
The [XDSMizer][gemseo.util.xdsm.xdsmizer.XDSMizer] generates a JSON file.
...
from gemseo.util.xdsm.html import generate_xdsm_html
from gemseo.util.xdsm.xdsm import XDSM
from gemseo.util.xdsm.xdsm_to_pdf import xdsm_data_to_pdf
...
class XDSMizer:
        ...
        Args:
            ...
            expected_workflow: The expected workflow,
                ...
                [EvaluationScenario][gemseo.scenario.evaluation.EvaluationScenario],
                 [BaseMDA][gemseo.mda.core.base.BaseMDA], etc.)
        """  # noqa:D205 D212 D415
    ...
    def _create_edges(self) -> list[EdgeType]:
        ...
        to_opt = list(self.scenario.formulation.problem.input_space.variables)
        ...
        for atom in self.atoms:
            if atom is not self.root_atom:
                ...
                varnames = sorted(
                    ...
                    & set(self.scenario.formulation.problem.input_space.variables)
                )
