...
class ScalableDiagonalModel(ScalableModel):
    ...
    def __init__(
        ...
        :param bool allow_unused_input: possibility to have an input
        ...
        if isinstance(fill_factor, Number):
            fill_factor = {
                function_name: fill_factor for function_name in data.outputs_names
            }
    ...
    def build_model(self):
        ...
        approx = ScalableDiagonalApproximation
        scalable_approximation = approx(
            self.sizes, self.lower_bounds, self.upper_bounds, comp_dep, inpt_dep, seed
        )
        return scalable_approximation

    def __build_scalable_functions(self):
        """ Builds all the required functions from the original cache. """
        t_scaled = {}
        f_scaled = {}
    ...
    def __convert_dependency_to_array(self, dependency):
        """Convert a dependency object of type dictionary into a dependency
    ...
    def plot_dependency(
        ...
        """This method plots the dependency matrix of a discipline
        in the form of a chessboard,
        where rows represent inputs, columns represent output and
        gray scale represent the dependency level between inputs and outputs.
        ...
        for pos in inputs_positions[1:]:
            axes.axhline(y=pos - 0.5, color="w", linewidth=3.0)
        for pos in outputs_positions[1:]:
            axes.axvline(x=pos - 0.5, color="w", linewidth=3.0)
    ...
    def generate_random_dependency(self):
        ...
        :param bool force_input_dependency: force input dependency for a given
            output. Default: True.
        :param bool allow_unused_inputs: allow unused inputs, that is to say
            input dimensions without any interaction with output functions.
            Default: False.
        :param dict(list(str)) io_dependency: input-output dependency
            structure.
            If None, all output components can depend on all input components.
            Default: None.
        :return: output component dependency and input-output dependency
        :rtype: dict(int), dict(dict(array))
        """
        npr.seed(self.parameters["seed"])
            ...
            for input_name in self.inputs_names:
                ...
                if input_name in io_dependency[function_name]:
                    if 0 <= fill_factor <= 1:
                        ...
                        rand_dep = npr.rand(function_size, input_size)
    ...
    def __generate_random_output_map(self):
        """Generate the dependency between the original and new output
        components for the different outputs.
        ...
        for function_name in self.outputs_names:
            ...
            function_size = self.sizes.get(function_name)
            out_map[function_name] = npr.randint(
                original_function_size, size=function_size
            )
        return out_map

    def __complete_random_dep(self, r_io_dep, dataname, index, io_dep):
        """Complete random dependency if row (input name)
        or column (function name) of the random dependency matrix is empty

        :param array inpt_dep: input-output dependency.
        ...
        if sum(inpt_dep_mat[index, :]) == 0:
            ...
            id_var = npr.choice(len(varnames), p=prob)
            id_comp = npr.randint(0, self.sizes.get(varnames[id_var], 1))
            if is_input:
...
class ScalableDiagonalApproximation(object):
    """
    Methodology that captures the trends of a physical problem, and
    extends it into a problem that has scalable input and outputs dimensions
    ...
    def __init__(self, sizes, var_lb, var_ub, output_dependency, io_dependency, seed=0):
        """
        Constructor:

        :param sizes: sizes of both input and output variables.
        :type sizes: dict
        :param var_lb: lower bounds of the input variables.
        :type var_lb: dict
        :param var_ub: upper bounds of the input variables.
        :type var_ub: dict
    ...
    def build_scalable_function(self, function_name, dataset, input_names, degree=3):
        """Build interpolation interpolation from a 1D input and output
        function. Add the model to the local dictionary

        :param str function_name: name of the output function
        :param AbstractFullCache dataset: the input-output dataset
        :param list(str) input_names: names of the input variables
        :param int degree: degree of interpolation (Default value = 3)
        """
        # 1) Get the unscaled data
        x_unscaled = []
        f_unscaled = []
        for data in dataset.get_all_data(True):
            f_unscaled.append(data[dataset.OUTPUTS_GROUP][function_name])
            x_unscaled.append(data[dataset.INPUTS_GROUP])

        # 2) Scale the input samples: [a1, b1] x ... x [an, bn] -> [0, 1]^n
        x_scaled = []
        for input_data in x_unscaled:
            squared_scaled_input_data = []
            for varname, value in input_data.items():
                scaled_value = value - self.var_lb[varname]
                scaled_value /= self.var_ub[varname] - self.var_lb[varname]
                squared_scaled_value = scaled_value ** 2.0
                squared_scaled_value = nan_to_num(squared_scaled_value)
                squared_scaled_input_data += list(squared_scaled_value)
            x_scaled.append(squared_scaled_input_data)

        # 3) Map the scaled input samples to [0,1]
        t_scaled = [atleast_1d(sqrt(mean(val))) for val in x_scaled]

        # 4) Sort t_scaled and f_unscaled following the t_scaled ascending
        # order
        indices = argsort([val[0] for val in t_scaled])
        t_scaled = [t_scaled[index] for index in indices]
        f_unscaled = [f_unscaled[index] for index in indices]

        # 5) scale the output samples: [a1, b1] x ... x [am, bm] -> [0, 1]^m
        f_scaled = self.scale_samples(f_unscaled)

        # 6) interpolate the (t_scaled, f_scaled) data
        self._interpolate(function_name, t_scaled, f_scaled, degree)

        # 7) compute the total input and output sizes
    ...
    def get_scalable_function(self, output_function):
        """Retrieve the scalable function generated from
    ...
    def get_scalable_derivative(self, output_function):
        """Retrieve the (scalable) gradient of the scalable function generated
    ...
    def _extrapolate(self, function_name, input_names, input_size, output_size):
        ...
        Generate a function that produces an output with a given size from an
        input with a given size, and its derivative.
        ...
        def scalable_derivative(input_vars):
            """n-dimensional to size_output-dimensional extrapolated
            function Jacobian
